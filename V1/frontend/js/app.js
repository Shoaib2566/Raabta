// =============================================================================
// app.js — Raabta Frontend Interactions
// Fully connected to backend via REST API (JWT authenticated)
// =============================================================================
...
(function () {
    ...
    // =========================================================================
    // NAVIGATION — Page & Preview Bar Helpers
    // =========================================================================

    window.setPB = function (btn) {
        document.querySelectorAll('.pb-btn').forEach(b => b.classList.remove('cur'));
        if (btn) btn.classList.add('cur');
    };

    window.setPBByPage = function (id) {
        const map = {
            'pg-home': 0,
            'pg-auth': 1,
            'pg-customer': 2,
        ...
        const btns = document.querySelectorAll('.pb-btn');
        btns.forEach(b => b.classList.remove('cur'));
        if (map[id] !== undefined && btns[map[id]]) {
            btns[map[id]].classList.add('cur');
        }
    };
    ...
    window.goPage = function (id) {
        ...
        setPBByPage(id);
        nativeScroll({ top: 0, left: 0, behavior: 'auto' });
    };
    ...
    window.scrollTo = function (arg) {
        if (typeof arg === 'string') return scrollToSection(arg);
        return nativeScroll(arg);
    };
    ...
    window.authTab = function (mode, btn) {
        ...
        } else if (tabs[mode === 'login' ? 0 : 1]) {
            tabs[mode === 'login' ? 0 : 1].classList.add('active');
        }

        byId('ap-' + mode)?.classList.add('active');

        if (byId('auth-heading')) {
            byId('auth-heading').textContent = mode === 'login' ? 'Welcome Back' : 'Create Account';
        }
        if (byId('auth-sub')) {
            byId('auth-sub').textContent = mode === 'login'
                ? 'Sign in to your Raabta account'
                : 'Join thousands using Raabta';
        }
    };


    // =========================================================================
    // API HELPER — Authenticated Fetch (Attaches JWT Token)
    // =========================================================================
    ...
    window.updateAvatars = function () {
        ...
        try {
            ...
            // Fallback to 'name' or 'full_name' depending on login/signup response
            ...
            // Update all dashboard avatars at once
            ['user-avatar', 'admin-avatar', 'supervisor-avatar'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.textContent = initials;
            });
        } catch (e) {
            console.error('Could not set avatar:', e);
        }
    };
    ...
    /**
     * Redirects the user to their role-specific dashboard after a successful
     * login or signup.
     */
    ...
    // =========================================================================
    // SECTION SWITCHER — Shared Helper for Dashboard Tabs
    // =========================================================================

    function showSection(scope, id, link, titleId, titles) {
        document.querySelectorAll(`#${scope} .dsection`).forEach(s => s.classList.remove('active'));
        document.querySelectorAll(`#${scope} .sb-link`).forEach(a => a.classList.remove('active'));
        byId(id)?.classList.add('active');
        if (link) link.classList.add('active');
        if (byId(titleId) && titles[id]) byId(titleId).textContent = titles[id];
    }
    ...
    window.cShow = function (id, link) {
        ...
        if (!link) {
            ...
            link = i >= 0 ? document.querySelectorAll('#pg-customer .sb-link')[i] : null;
        }
        showSection('pg-customer', id, link, 'c-title', {
            'c-dash':       'Dashboard',
            'c-neworder':   'New Order',
            'c-services':   'Services',
            'c-history':    'Order History',
            'c-plans':      'My Plan',
            'c-complaints': 'Complaints',
            'c-profile':    'Profile'
        });

        // Trigger data fetches on tab switch
        if (id === 'c-dash')       { loadCustomerDashboard(); loadCustomerPlan(); }
        if (id === 'c-history')    loadCustomerOrders();
        if (id === 'c-complaints') loadCustomerComplaints();
        if (id === 'c-plans')      loadCustomerPlan();
    };
    ...
    window.aShow = function (id, link) {
        ...
        if (!link) {
            const i = ids.indexOf(id);
            link = i >= 0 ? document.querySelectorAll('#pg-admin .sb-link')[i] : null;
        }
        showSection('pg-admin', id, link, 'a-title', {
            'a-services':   'Services',
            'a-analytics':  'Analytics',
            'a-users':      'User Management',
            'a-logs':       'Activity Logs',
    ...
    // --- Supervisor Section Switcher ---
    window.sShow = function (id, link) {
        const ids = ['s-orders', 's-assign', 's-status', 's-providers'];
        if (!link) {
            const i = ids.indexOf(id);
            link = i >= 0 ? document.querySelectorAll('#pg-supervisor .sb-link')[i] : null;
        }
        showSection('pg-supervisor', id, link, 's-title', {
            's-orders':    'Orders',
            's-assign':    'Assign Order',
            's-status':    'Update Status',
            's-providers': 'Provider Directory'
        });

        // Trigger data fetches on tab switch
        if (id === 's-orders')    loadSupervisorOrders();
        if (id === 's-providers') loadSupervisorProviders();
        if (id === 's-assign')    { loadSupervisorProviders(); loadSupervisorAssignments(); }
    };


    // =========================================================================
    // MODAL HELPERS — Rating, Order Tracking, Complaints
    // =========================================================================

    window.openRate  = () => byId('rate-modal')?.classList.add('open');
    window.closeRate = () => byId('rate-modal')?.classList.remove('open');
    ...
    // --- Admin / Supervisor Modal Triggers ---
    window.openAddSvc  = () => byId('addsvc-modal')?.classList.add('open');
    window.closeAddSvc = () => byId('addsvc-modal')?.classList.remove('open');

    window.sGoAssign = function (id, svc) {
        if (byId('s-assign-info')) byId('s-assign-info').textContent = `${id} — ${svc}`;
        sShow('s-assign');
    };

    window.sGoStatus = function (id) {
        if (byId('s-status-info')) byId('s-status-info').textContent = id;
        sShow('s-status');
    };


    // =========================================================================
    // CUSTOMER — Dashboard, Orders, Services & Plan
    // =========================================================================

    window.loadCustomerDashboard = async function () {
        try {
            // Clear stale data immediately so the user sees a loading state
            ...
            if (orderList) {
                orderList.innerHTML = '<p style="color:var(--muted);font-size:0.875rem;">Loading real data from Supabase...</p>';
            }
            ...
            if (welcomeH2) {
                welcomeH2.innerHTML = `Good afternoon, ${data.profile.full_name.split(' ')[0]} 👋`;
            }
        ...
        try {
            ...
            orders.forEach(order => {
                const statusClass = order.status === 'completed'
                    ? 'b-green'
                    : (['requested', 'assigned'].includes(order.status) ? 'b-amber' : 'b-blue');
                const displayStatus = order.status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
    ...
    // Populate the New Order service dropdown from the database
    ...
    window.submitNewOrder = async function () {
        ...
        if (serviceSelect.selectedIndex === 0 || !cityInput) {
            alert('Please select a service and enter your city.');
            return;
        }
        ...
        try {
            ...
            if (e.message.includes('free limit reached')) {
                alert('Free limit reached. Upgrade your plan to proceed further.');
            } else {
    ...
    window.loadCustomerPlan = async function () {
        try {
            ...
            // Animate usage progress bar
            const bar = byId('plan-usage-bar');
            if (bar) {
                let percentage = Math.min((data.usage / data.limit) * 100, 100);
    ...
    window.trackOrder = async function (presetId) {
        ...
        try {
            ...
            if (stepsContainer) {
                ...
                const stepText = [
                    'Requested — Order placed and confirmed',
                    'Assigned — Provider identified and contacted',
                    'In Progress — Work underway at your location',
                    ...
                    if (i < statusLevel) {
                        stepsHtml += `
                            <div style="display:flex;align-items:center;gap:.75rem;font-size:.875rem">
                                <span style="width:22px;height:22px;background:var(--g500);border-radius:50%;display:flex;align-items:center;justify-content:center;color:white;font-size:.7rem;flex-shrink:0">✓</span>
                                <span style="color:var(--g700)">${stepText[i - 1]}</span>
                            </div>`;
                    } else if (i === statusLevel) {
                        stepsHtml += `
                            <div style="display:flex;align-items:center;gap:.75rem;font-size:.875rem">
                                <span style="width:22px;height:22px;background:var(--gold);border-radius:50%;display:flex;align-items:center;justify-content:center;color:var(--g900);font-size:.7rem;flex-shrink:0;font-weight:700">→</span>
                                <span style="color:var(--g800);font-weight:600">${stepText[i - 1]}</span>
                            </div>`;
                    } else {
                        stepsHtml += `
                            <div style="display:flex;align-items:center;gap:.75rem;font-size:.875rem">
                                <span style="width:22px;height:22px;background:var(--border);border-radius:50%;display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:.7rem;flex-shrink:0">○</span>
                                <span style="color:var(--muted)">${stepText[i - 1]}</span>
                            </div>`;
                    }
                }
    ...
    window.submitComplaint = async function () {
        ...
        if (!typeInput || !detailsInput) {
            alert('Please select a complaint type and provide details.');
            return;
        }
        ...
        try {
            ...
            if (tbody) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${orderInput || 'N/A'}</td>
                    <td>${typeInput}</td>
                    <td><span class="badge b-amber">Open</span></td>`;
                tbody.prepend(tr);
            }

            ['complaint-order', 'complaint-type', 'complaint-details'].forEach(x => {
                if (byId(x)) byId(x).value = '';
            });
        } catch (e) {
    ...
    window.loadAdminAnalytics = async function () {
        try {
            ...
            // Orders by Service — horizontal bars
            ...
            // Order Status Breakdown — separated stacked bars
            ...
            if (byId('analytics-chart-bars')) {
                byId('analytics-chart-bars').innerHTML = '<div style="color:var(--muted)">Failed to load data</div>';
            }
        }
    };
    ...
    window.loadAdminUsers = async function () {
        try {
            ...
            // Cache globally so the filter function doesn't need to re-fetch
            ...
            const tbody = document.getElementById('a-users-tbody');
            if (tbody) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:1.5rem">Failed to load data. Is the backend running?</td></tr>';
            }
        }
    };
    ...
    window.saveAddUser = async function () {
        ...
        try {
            // Reuse auth/signup so password hashing is handled consistently
    ...
    window.saveEditUser = async function () {
        ...
        try {
            await fetchWithAuth(`/admin/users/${window.currentEditingUserId}`, {
                method: 'PATCH',
                body: JSON.stringify({ full_name: name, email, role, account_status: status })
            });
            closeEditUser();
            loadAdminUsers();
        } catch (error) {
    ...
    window.saveEditSvc = async function () {
        ...
        try {
            await fetchWithAuth(`/admin/services/${window.currentEditingSvcId}`, {
                method: 'PATCH',
                body: JSON.stringify({ service_name: name, category, description: desc })
            });
            closeEditSvc();
            loadAdminServices();
        } catch (error) {
        ...
        try {
            ...
            orders.forEach(o => {
                ...
                if (o.status === 'completed') {
                    if (new Date(o.requested_at).toDateString() === todayStr) completedTodayCount++;
                }
            });
    ...
    // =========================================================================
    // SUPERVISOR — Assign & Status Actions
    // =========================================================================
    ...
    window.doAssign = async function () {
        ...
        if (!order_id || !provider_id) {
            alert('Please select an order and a provider.');
            return;
        }
    ...
    window.doStatus = async function () {
        ...
        if (!order_id || !new_status) {
            alert('Please select an order and a new status.');
            return;
        }
        ...
        try {
            ...
            document.querySelector('#s-status input[type="text"]').value = '';
            sShow('s-orders');
        } catch (e) {
    ...
    window.loadSupervisorProviders = async function () {
        try {
            ...
            // Ensure a dedicated, safe card container exists
            let cardContainer = document.getElementById('prov-card-container');
            if (directoryContainer && !cardContainer) {

                // Remove all dummy/stale HTML except the Add Provider button
                Array.from(directoryContainer.children).forEach(child => {
                    if (!child.innerHTML?.includes('+ Add Provider')) child.remove();
                });
                // Remove any bare stray text nodes
                ...
                // Create a clean, easily re-renderable container
            ...
            // Clear container after fetch to avoid stale content
    ...
    window.toggleSupNotifs = function () {
        ...
        if (drop.classList.contains('show')) loadSupervisorNotifications();
    };
    ...
    // =========================================================================
    // INITIALIZATION — Navigation Binding & DOMContentLoaded
    // =========================================================================

    function bindNavigation() {
        // Top nav links → smooth scroll to anchors
        document.querySelectorAll('.nav-links a').forEach(a => {
            const t = a.textContent.toLowerCase();
            if (t.includes('about'))    a.onclick = e => { e.preventDefault(); scrollToSection('about-anchor');    };
            if (t.includes('services')) a.onclick = e => { e.preventDefault(); scrollToSection('services-anchor'); };
            if (t.includes('pricing'))  a.onclick = e => { e.preventDefault(); scrollToSection('pricing-anchor');  };
        });

        // Preview bar buttons → page navigation
        document.querySelectorAll('.preview-bar .pb-btn').forEach(btn => {
            btn.onclick = e => {
                e.preventDefault();
                const t = btn.textContent;
                if      (t.includes('Home'))       goPage('pg-home');
                else if (t.includes('Auth'))       goPage('pg-auth');
                else if (t.includes('Customer'))   goPage('pg-customer');
                else if (t.includes('Admin'))      goPage('pg-admin');
                else if (t.includes('Supervisor')) goPage('pg-supervisor');
            };
        });

        // Explore Services button
        const explore = [...document.querySelectorAll('button')].find(b => b.textContent.includes('Explore Services'));
        if (explore) explore.onclick = e => { e.preventDefault(); scrollToSection('services-anchor'); };
    }

    // Global delegated click handler for sidebar navigation and Add Provider
    document.addEventListener('click', e => {
        ...
        if (target.closest('#pg-customer .sb-nav')) {
            const links = [...document.querySelectorAll('#pg-customer .sb-link')];
            const ids   = ['c-dash', 'c-neworder', 'c-services', 'c-history', 'c-plans', 'c-complaints', 'c-profile'];
        ...
        if (target.closest('#pg-admin .sb-nav')) {
            const links = [...document.querySelectorAll('#pg-admin .sb-link')];
            const ids   = ['a-services', 'a-analytics', 'a-users', 'a-logs', 'a-complaints'];
            const i     = links.indexOf(target);
        ...
        if (target.closest('#pg-supervisor .sb-nav')) {
            const links = [...document.querySelectorAll('#pg-supervisor .sb-link')];
            const ids   = ['s-orders', 's-assign', 's-status', 's-providers'];
            const i     = links.indexOf(target);
            if (i >= 0) {
                e.preventDefault();

                // Reset info banners when navigating to assign/status tabs directly
        ...
        // Add Provider button (delegated)
        if (target.textContent.trim() === '+ Add Provider') {
            e.preventDefault();
            openAddProvider();
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Override any inline onclick on the New Order submit button
        ...
        // Update top nav auth button if already logged in
        ...
        // Auto-login: if a valid session is stored, redirect immediately
        const token      = localStorage.getItem('raabta_token');
        const userString = localStorage.getItem('raabta_user');
        if (token && userString) {
            updateAvatars();
            const user = JSON.parse(userString);
            if      (user.role === 'admin')      { goPage('pg-admin');      aShow('a-analytics'); }
            else if (user.role === 'supervisor') { goPage('pg-supervisor'); sShow('s-orders');    }
            else                                 { goPage('pg-customer');   cShow('c-dash');       }
        }
    });
